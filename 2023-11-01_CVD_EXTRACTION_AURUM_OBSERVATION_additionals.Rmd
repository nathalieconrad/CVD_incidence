---
title: "CVD Incidence - AURUM OBSERVATION TABLE"
...
study_end_date = as.Date("31/12/2018", format="%d/%m/%Y")
...
```{r, eval=TRUE, warning=FALSE, message=FALSE}
## Read code tables
comorb_codes = fread(file.path(PATH, "Phenotyping/Comorbidities/2022-02-03_FINAL_Comorbidities_diagnostic_codes_table.txt"))
nrow(comorb_codes)
head(comorb_codes)

comorb_codes = comorb_codes[include==1]
levels(as.factor(comorb_codes$disease))
head(comorb_codes)
nrow(comorb_codes)

## Define codetype
levels(as.factor(comorb_codes$codetype))

## Subset for Read codes in Aurum
comorb_read_codes = subset(comorb_codes, codetype %in% c("aurum"), select =c(disease, codetype, unique_ID, medcode_aurum, description, include))
class(comorb_read_codes$medcode_aurum)
head(comorb_read_codes)
#comorb_read_codes[,unique_ID_orig := unique_ID] 
#comorb_read_codes[,unique_ID := paste("A",medcode_aurum, sep="_")] 
head(comorb_read_codes)
comorb_codes = comorb_read_codes
```
...
colnames(ethnicity_codes_gold_aurum)
nrow(ethnicity_codes_gold_aurum)
```

## ALL codes 
```{r, eval=TRUE, warning=FALSE, message=FALSE}
all_obs_codes = rbind(comorb_codes, obs_codes, ethnicity_codes_gold_aurum, fill =TRUE)
nrow(all_obs_codes)
head(all_obs_codes)
```
...
observation_aurum = rbindlist( lapply(list.files(), function(x) { subset(fread(x), medcodeid %in% all_obs_codes$medcode_aurum) } ))
...
fwrite(observation_aurum1, file.path(PATH_W,"observation_aurum1.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
```
...
fwrite(observation_aurum2, file.path(PATH_W,"observation_aurum2.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum3, file.path(PATH_W,"observation_aurum3.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum4, file.path(PATH_W,"observation_aurum4.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum5, file.path(PATH_W,"observation_aurum5.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum6, file.path(PATH_W,"observation_aurum6.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum7, file.path(PATH_W,"observation_aurum7.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum8, file.path(PATH_W,"observation_aurum8.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum9, file.path(PATH_W,"observation_aurum9.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum10, file.path(PATH_W,"observation_aurum10.txt.gz")) # Save table
rm(list=setdiff(ls(), c("aurum_main_diseases_matched","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
...
fwrite(observation_aurum11, file.path(PATH_W,"observation_aurum11.txt.gz")) # Save table
rm(list=setdiff(ls(), c("study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
```
...
# Set 1
start_time <- Sys.time()
observation_aurum1 = fread(file.path(PATH_W,"observation_aurum1.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum2 = fread(file.path(PATH_W,"observation_aurum2.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum3 = fread(file.path(PATH_W,"observation_aurum3.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum4 = fread(file.path(PATH_W,"observation_aurum4.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum5 = fread(file.path(PATH_W,"observation_aurum5.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
...
# Set 2
#start_time <- Sys.time()
observation_aurum6 = fread(file.path(PATH_W,"observation_aurum6.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum7 = fread(file.path(PATH_W,"observation_aurum7.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum8 = fread(file.path(PATH_W,"observation_aurum8.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum9 = fread(file.path(PATH_W,"observation_aurum9.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum10 = fread(file.path(PATH_W,"observation_aurum10.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
observation_aurum11= fread(file.path(PATH_W,"observation_aurum11.txt.gz"), colClasses=c("patid"="integer64", "obsdate"="Date")) 
...
observation_aurum_set = rbind(observation_aurum1, observation_aurum2, observation_aurum3, observation_aurum4, observation_aurum5, observation_aurum6, observation_aurum7, observation_aurum8, observation_aurum9, observation_aurum10, observation_aurum11)
fwrite(observation_aurum_set, file.path(PATH_W,"observation_aurum_additionals.txt.gz")) # Save table
rm(list=setdiff(ls(), c("observation_aurum_set","study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_L", "all_codes_table", "comorb_codes", "comorb_read_codes", "comorb_icd_codes", "start_time", "all_obs_codes")))
gc()
end_time <- Sys.time()
end_time - start_time 
```
