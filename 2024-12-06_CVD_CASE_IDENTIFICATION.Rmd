...
include_procedures = "yes" # select yes if procedures are to be included 
```
...
## Disease codes - (code review 18/01/2024)
```{r disease_codes, eval=TRUE, warning=FALSE, message=FALSE}
# all codes
all_codes_table = data.table(read.table(file.path(PATH,"/Phenotyping/Final_AD_INF_CVD/CVD_incidence/2024-01-19_FINAL_CVD_diagnostic_codes_table_JMM.txt"), header = TRUE, sep="\t", colClasses=c("medcode_aurum"="integer64"))) # Final set of codes for analysis as per 19/01/2024

nrow(all_codes_table)
head(all_codes_table)
levels(as.factor(all_codes_table$include))

#include_procedures ---- CHECK
...
fwrite(denominator, file.path(PATH_W_CVD, "denominator_first_release.txt.gz"))

```

## 1.4 Denominator characteristics
```{r, eval=TRUE, warning=FALSE, message=FALSE}
# Included in the study were men and women with records labelled as ‘acceptable', approved for HES and ONS linkage, and registered with their general practice for at least 12 months during the study period (01/01/ 1998 to 30/06/2019). 
#denominator = fread(file.path(PATH_W_CVD, "denominator_first_release.txt.gz"))
...
gold_diagnoses_all = rbind( gold_diagnoses_primary_care_dict, gold_diagnoses_HES_APC_dict,gold_diagnoses_HES_OP_dict[,-("pracid")], death_gold_melted_cvd, fill=TRUE)
...
# And add denominator information
format(nrow(gold_diagnoses_all), big.mark="'")  # number of records in table
gold_diagnoses_all = merge(gold_diagnoses_all, denominator_gold, by = "patid", all.x = TRUE)
format(nrow(gold_diagnoses_all), big.mark="'")  # number of records in table
nrow(denominator_gold[is.na(exclude)])
...
format(nrow(gold_diagnoses_all), big.mark="'") 
summary(gold_diagnoses_all$pracid)
colnames(gold_diagnoses_all)
...
```{r, eval=TRUE, warning=FALSE, message=FALSE}
aurum_diagnoses_primary_care= fread(file.path(PATH_W_CVD,"/observation_aurum_diagnoses.txt.gz"), colClasses=c("patid"="integer64", "medcodeid"="integer64", "obsdate"="Date"))
...
# Rbind tables
aurum_diagnoses_all = rbind( aurum_diagnoses_primary_care_dict, aurum_diagnoses_HES_APC_dict,aurum_diagnoses_HES_OP_dict, death_aurum_melted_cvd,  fill=TRUE ) 
nrow(aurum_diagnoses_all)

# Add denominator information
format(nrow(aurum_diagnoses_all), big.mark="'")  # number of records in table
aurum_diagnoses_all = merge(aurum_diagnoses_all, denominator_aurum, by = "patid", all.x = TRUE)
format(nrow(aurum_diagnoses_all), big.mark="'")  # number of records in table
summary(aurum_diagnoses_all$pracid)
colnames(aurum_diagnoses_all)
...
#save.image(file.path(PATH_W_CVD,"2022-02-18FullWorkspaceIdentification.RData"))
fwrite(diagnoses_all, file.path(PATH_W_CVD, "CVD_diagnoses_all.txt.gz"))
fwrite(death_melted_cvd, file.path(PATH_W_CVD, "CVD_death_melted.txt.gz"))
fwrite(hes_primary, file.path(PATH_W_CVD, "CVD_hes_primary.txt.gz"))
fwrite(death_all, file.path(PATH_W_CVD, "CVD_death_all.txt.gz"))

rm(list=setdiff(ls(), c("diagnoses_all", "death_melted_cvd", "hes_primary", "diagnoses_all_0", "study_start_date", "study_end_date", "PATH", "PATH_D", "PATH_D2", "PATH_W_CVD","PATH_W_all", "PATH_L", "gold_codes_table", "aurum_codes_table", "icd_codes_table", "icd_9_10_codes_table", "opcs_codes_table", "all_codes_table", "denominator", "age_group_table", "compute_days", "age_group_fun", "icd_codes_to_be_double_checked_for_inclusion_gold", "diag_codes_table_inc_0", "gold_codes_to_be_double_checked_for_inclusion", "aurum_codes_to_be_double_checked_for_inclusion", "icd_codes_to_be_double_checked_for_inclusion_aurum", "include_procedures")))
save.image(file.path(PATH_W_CVD,"2024-01-19_SelectedWorkspaceIdentification_CVD_1.RData"))
#load(file.path(PATH_W_CVD,"2022-02-18SelectedWorkspaceIdentification_2.RData"))
gc()
```
...
fwrite(diagnoses_all_first, file.path(PATH_W_CVD, "CVD_diagnoses_first.txt.gz"))

rm(list=setdiff(ls(), c("diagnoses_all_first", "diagnoses_all_AD_first", "diagnoses_all_CVD_first","study_start_date", "study_end_date", "PATH", "PATH_D", "PATH_D2", "PATH_W_CVD", "PATH_W_all", "PATH_L", "gold_codes_table", "aurum_codes_table", "icd_codes_table", "icd_9_10_codes_table", "opcs_codes_table", "all_codes_table", "denominator", "cohort_1", "age_group_table", "compute_days", "age_group_fun", "icd_codes_to_be_double_checked_for_inclusion_gold", "diag_codes_table_inc_0", "gold_codes_to_be_double_checked_for_inclusion", "aurum_codes_to_be_double_checked_for_inclusion", "icd_codes_to_be_double_checked_for_inclusion_aurum", "include_procedures")))
#load(file.path(PATH_W_CVD,"2022-02-18SelectedWorkspaceIdentification_3.RData"))
#save.image(file.path(PATH_W_CVD,"2022-02-18SelectedWorkspaceIdentification_3.RData"))
gc()
```
...
# Save_4
```{r save_4, eval=TRUE, warning=FALSE, message=FALSE}
#save.image(file.path(PATH_W_CVD,"2022-02-18FullWorkspaceIdentification.RData"))
rm(list=setdiff(ls(), c("diagnoses_all_first_denom", "matching_cohort_no_CVD", "study_start_date", "study_end_date", "PATH", "PATH_D", "PATH_D2", "PATH_W_CVD","PATH_W_all", "PATH_L", "gold_codes_table", "aurum_codes_table", "icd_codes_table", "icd_9_10_codes_table", "opcs_codes_table", "all_codes_table", "denominator", "cohort_1", "age_group_table", "compute_days", "age_group_fun", "icd_codes_to_be_double_checked_for_inclusion_gold", "diag_codes_table_inc_0", "gold_codes_to_be_double_checked_for_inclusion", "aurum_codes_to_be_double_checked_for_inclusion", "icd_codes_to_be_double_checked_for_inclusion_aurum", "include_procedures")))

save.image(file.path(PATH_W_CVD,"2024-01-19_SelectedWorkspaceIdentification_CVD_4.RData"))
```