...
```{r , eval=TRUE, warning=FALSE, message=FALSE}
# all first diagnoses
diagnoses_all_first_denom= fread(file.path(PATH_W,"diagnoses_all_first_denom.txt.gz"), colClasses=c("patid"="integer64", "eventdate"="Date", "first_AD_date"="Date" , "first_INF_date"="Date", "first_HER_date"="Date", "first_CVD_date"="Date", "first_CVD_hes_primary_date"="Date", "regdate"="Date", "end_date"="Date", "start_date"="Date", "lcdate"="Date", "todate"="Date")) # all first diagnoses for diseases included in the study
...
diagnoses_all_first_denom[,diag_date:= eventdate]
diagnoses_all_first_denom[,row_ID:= paste(db_patid,"_", disease_short, sep="")]
colnames(diagnoses_all_first_denom)

# Define row_ID (unique patid-disease tuple)
diagnoses_all_first_denom[,row_ID := paste(db_patid, disease_short, sep ="_")]
format(nrow(diagnoses_all_first_denom), big.mark="'")  
diagnoses_all_first_denom

diagnoses_all_first_denom = diagnoses_all_first_denom[!is.na(eventdate)]
diagnoses_all_first_denom[,diag_date:=eventdate]
format(nrow(diagnoses_all_first_denom), big.mark="'")  

# subset GOLD patients
diagnoses_all_first_denom_gold = diagnoses_all_first_denom[substr(db_patid,1,1) == "G"]
format(nrow(diagnoses_all_first_denom_gold), big.mark="'")  

# subset Aurum patients
diagnoses_all_first_denom_aurum = diagnoses_all_first_denom[substr(db_patid,1,1) == "A"]
format(nrow(diagnoses_all_first_denom_aurum), big.mark="'")  

nrow(diagnoses_all_first_denom) - nrow(diagnoses_all_first_denom_gold) - nrow(diagnoses_all_first_denom_aurum) # check that this is 0
...
comorb_codes_table_1[disease == "Type 2 diabetes"]
#nrow(diag_codes_table_1)
...
clinical_gold = clinical_gold[patid %in% diagnoses_all_first_denom_gold$patid] 
format(nrow(clinical_gold), big.mark="'") 
```
...
# Subset to patient in present analysis set
additionals_gold = additionals_gold[patid %in% diagnoses_all_first_denom_gold$patid]
...
#additionals_gold = merge(x=additionals_gold, y=unique(diagnoses_all_first_denom_gold[,.(patid, birthyear)]), by = c("patid"), all.x = TRUE)
...
## Get diagnosis date
```{r, eval=TRUE, warning=FALSE, message=FALSE}
diagnoses_all_first_denom_gold
additionals_gold = merge(x=additionals_gold, y=unique(diagnoses_all_first_denom_gold[,.(patid, disease, diag_date, row_ID)]), by = c("patid"), all.x = TRUE, allow.cartesian = TRUE)
format(nrow(additionals_gold), big.mark="'") 
...
# Subset to patient in present analysis set
test_gold_chol = test_gold_chol[patid %in% diagnoses_all_first_denom_gold$patid] 
format(nrow(test_gold_chol), big.mark="'") 
...
test_gold_chol = merge(x=test_gold_chol, y=unique(diagnoses_all_first_denom_gold[,.(patid, disease, diag_date, row_ID)]), by = c("patid"), all.x = TRUE, allow.cartesian = TRUE)
...
# Subset to patient in present analysis set
comorb_gold = comorb_gold[patid %in% unique(diagnoses_all_first_denom_gold$patid)] 
...
comorb_gold = merge(comorb_gold, unique(comorb_codes_table_1[,.(disease, codetype, description, unique_ID)]), by ="unique_ID", all.x = TRUE, allow.cartesian=TRUE)
head(comorb_gold)
...
comorb_gold = merge(comorb_gold, diagnoses_all_first_denom_gold[,.(patid, row_ID, diag_date)], by = "patid", all.x =TRUE, allow.cartesian = TRUE)
head(comorb_gold)
...
comorb_gold_first = comorb_gold_first[patid %in% unique(diagnoses_all_first_denom_gold$patid)] # subset to patient in present analysis set
...
disease_short = data.table(disease = c("Cancer","CKD", "Type 2 diabetes", "Dyslipidaemia" , "Hypertension", "Obesity", "Psoriatic arthritis", "Hyperthyroidism"), disease_short = c("CAN", "CKD", "DIA2", "DYS", "HYP", "OBE", "PSA", "HTH"))
...
# save table
fwrite(comorb_gold_first_index_long, file.path(PATH_W,"CVD_gold_comorbidities.txt.gz")) 
...
gold_ethnicity_hes[ethnicity_description %in% c("Bl_Afric", "Bl_Carib", "Bl_Other"), ethnicity_category1 := "African/Caribean"]
...
format(nrow(diagnoses_all_first_denom_gold), big.mark="'")
format(nrow(unique(diagnoses_all_first_denom_gold[,.(row_ID)])), big.mark="'") 

# blood pressure
diagnoses_all_first_denom_add_gold = merge(x=diagnoses_all_first_denom_gold, y=blood_pressure_gold, by = "row_ID", all.x = TRUE) # where covariate depend on diagnosis date, merge by row_ID
head(diagnoses_all_first_denom_add_gold)
head(comorb_gold_first_index_long)

# weight
diagnoses_all_first_denom_add_gold = merge(x=diagnoses_all_first_denom_add_gold, y=weight_gold, by = "row_ID", all.x = TRUE)

# smoking
diagnoses_all_first_denom_add_gold = merge(x=diagnoses_all_first_denom_add_gold, y=smoking_gold, by = "row_ID", all.x = TRUE)

# cholesterol
diagnoses_all_first_denom_add_gold = merge(x=diagnoses_all_first_denom_add_gold, y=chol_gold, by = "row_ID", all.x = TRUE)
format(nrow(diagnoses_all_first_denom_add_gold), big.mark="'")

# comorbidities
diagnoses_all_first_denom_add_comord_gold = merge(x=diagnoses_all_first_denom_add_gold, y=comorb_gold_first_index_long, by = "row_ID", all.x = TRUE)
format(nrow(diagnoses_all_first_denom_add_comord_gold), big.mark="'") 
diagnoses_all_first_denom_add_comord_gold[is.na(CKD_any_time_flag), CKD_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(CAN_any_time_flag), CAN_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(DIA2_any_time_flag), DIA2_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(DYS_any_time_flag), DYS_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(HYP_any_time_flag), HYP_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(OBE_any_time_flag), OBE_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_gold[is.na(HTH_any_time_flag), HTH_any_time_flag:=0]  # for those without relevant diagnoses, set disease flag to 0
summary(diagnoses_all_first_denom_add_comord_gold$OBE_any_time_flag)
format(nrow(diagnoses_all_first_denom_add_comord_gold[is.na(OBE_any_time_flag)]), big.mark="'") # check that no flag is set to NA

# Set flags for comorbidity prevalence up to index date
diagnoses_all_first_denom_add_comord_gold[,CAN_flag := CAN_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[CAN_any_time_date > diag_date ,CAN_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,CKD_flag := CKD_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[CKD_any_time_date > diag_date ,CKD_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,DIA2_flag := DIA2_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[DIA2_any_time_date > diag_date ,DIA2_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,DYS_flag := DYS_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[DYS_any_time_date > diag_date ,DYS_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,HYP_flag := HYP_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[HYP_any_time_date > diag_date ,HYP_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,OBE_flag := OBE_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[OBE_any_time_date > diag_date ,OBE_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,PSA_flag := PSA_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[PSA_any_time_date > diag_date ,PSA_flag := 0]
diagnoses_all_first_denom_add_comord_gold[,HTH_flag := HTH_any_time_flag]; diagnoses_all_first_denom_add_comord_gold[HTH_any_time_date > diag_date ,HTH_flag := 0]

diagnoses_all_first_denom_add_comord_gold[,CAN_date := CAN_any_time_date]; diagnoses_all_first_denom_add_comord_gold[CAN_any_time_date > diag_date ,CAN_date := NA]
diagnoses_all_first_denom_add_comord_gold[,CKD_date := CKD_any_time_date]; diagnoses_all_first_denom_add_comord_gold[CKD_any_time_date > diag_date ,CKD_date := NA]
diagnoses_all_first_denom_add_comord_gold[,DIA2_date := DIA2_any_time_date]; diagnoses_all_first_denom_add_comord_gold[DIA2_any_time_date > diag_date ,DIA2_date := NA]
diagnoses_all_first_denom_add_comord_gold[,DYS_date := DYS_any_time_date]; diagnoses_all_first_denom_add_comord_gold[DYS_any_time_date > diag_date ,DYS_date := NA]
diagnoses_all_first_denom_add_comord_gold[,HYP_date := HYP_any_time_date]; diagnoses_all_first_denom_add_comord_gold[HYP_any_time_date > diag_date ,HYP_date := NA]
diagnoses_all_first_denom_add_comord_gold[,OBE_date := OBE_any_time_date]; diagnoses_all_first_denom_add_comord_gold[OBE_any_time_date > diag_date ,OBE_date := NA]
diagnoses_all_first_denom_add_comord_gold[,PSA_date := PSA_any_time_date]; diagnoses_all_first_denom_add_comord_gold[PSA_any_time_date > diag_date ,PSA_date := NA]
diagnoses_all_first_denom_add_comord_gold[,HTH_date := HTH_any_time_date]; diagnoses_all_first_denom_add_comord_gold[HTH_any_time_date > diag_date ,HTH_date := NA]

# ethnicity
diagnoses_all_first_denom_add_comord_gold = merge(x=diagnoses_all_first_denom_add_comord_gold, y=gold_ethnicity, by = "patid", all.x = TRUE, allow.cartesian = TRUE)
format(nrow(diagnoses_all_first_denom_add_comord_gold), big.mark="'") # where covariate is time-independent, merge by patid
...
# save table
fwrite(diagnoses_all_first_denom_add_comord_gold, file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord_gold.txt.gz")) 
rm(list=setdiff(ls(), c("study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_W_all", "PATH_L", "PATH_P", "obs_codes", "comorb_codes_table_1", "ethnicity_codes_gold_aurum", "diagnoses_all_first_denom_aurum", "comorb_gold_first", "comorb_aurum_first")))
gc()
```
...
observation_aurum = fread(file.path(PATH_W,"observation_aurum_additionals.txt.gz")) # this file only contains observations for the selected codes for weight, height, BMI, smoking, blood pressure, comorbidities and ethnicity for all patients
...
# Subset to patient in present analysis set
observation_aurum = observation_aurum[patid %in% diagnoses_all_first_denom_aurum$patid] 
format(nrow(observation_aurum), big.mark="'") 
```

## Get diag_date
```{r, eval=TRUE, warning=FALSE, message=FALSE}
observation_aurum = merge(x=observation_aurum, y=unique(diagnoses_all_first_denom_aurum[,.(patid, disease, diag_date, row_ID)]), by ="patid", all.x = TRUE, allow.cartesian = TRUE)
format(nrow(observation_aurum), big.mark="'") 
```
...
# save table
fwrite(blood_pressure_aurum, file.path(PATH_W,"CVD_blood_pressure_aurum.txt.gz")) 
```
...
# save table
fwrite(bmi_aurum, file.path(PATH_W,"CVD_bmi_aurum.txt.gz")) 
...
# save table
fwrite(smoking_aurum, file.path(PATH_W,"CVD_smoking_aurum.txt.gz")) 
```
...
# save table
fwrite(chol_aurum, file.path(PATH_W,"CVD_chol_aurum.txt.gz")) 
```
...
#comorb_aurum_pc = subset(fread(file.path(PATH_W,"observation_aurum_selected.txt.gz")), select = c("patid", "obsdate", "medcodeid"), medcodeid %in% comorb_codes_table_1[codetype == "aurum"]$medcode_aurum)
...
comorb_aurum = merge(comorb_aurum, unique(comorb_codes_table_1[,.(disease, description, unique_ID)]), by.x =c( "comorb_diag_code_unique_ID") , by.y =c("unique_ID"), all.x = TRUE, allow.cartesian = TRUE)
...
comorb_aurum_first = merge(comorb_aurum_first, diagnoses_all_first_denom_aurum[,.(patid, row_ID, diag_date)], by = "patid", all.x =TRUE, allow.cartesian=TRUE)
format(nrow(comorb_aurum_first), big.mark="'") 
...
# save table
fwrite(comorb_aurum_first_index_long, file.path(PATH_W,"CVD_aurum_comorbidities.txt.gz")) 
```

# AURUM - Ethnicity
```{r ethnicity_aurum, eval=TRUE, warning=FALSE, message=FALSE}
# Extract ethnicity from Aurum
#aurum_ethnicity_primary_care= subset(fread(file.path(PATH_W,"/observation_aurum_diagnoses.txt.gz"), colClasses=c("patid"="integer64", "medcodeid"="integer64", "obsdate"="Date")), medcodeid %in% ethnicity_codes_gold_aurum$medcode_aurum)
...
aurum_ethnicity_hes[ethnicity_description %in% c("Bl_Afric", "Bl_Carib", "Bl_Other"), ethnicity_category1 := "African/Caribean"]
...
# save table
fwrite(aurum_ethnicity, file.path(PATH_W,"CVD_aurum_ethnicity.txt.gz")) 
rm(list=setdiff(ls(), c("study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_W_all", "PATH_L", "PATH_P", "diagnoses_all_first_denom", "diagnoses_all_first_denom_aurum", "obs_codes", "comorb_codes_table_1", "ethnicity_codes_gold_aurum", "comorb_gold_first", "comorb_aurum_first", "comorb_aurum_first_index_long", "aurum_ethnicity", "blood_pressure_aurum", "bmi_aurum", "smoking_aurum", "chol_aurum")))
gc()
```
...
format(nrow(diagnoses_all_first_denom_aurum), big.mark="'")
format(nrow(unique(diagnoses_all_first_denom_aurum[,.(row_ID)])), big.mark="'") 

# blood pressure
diagnoses_all_first_denom_add_aurum = merge(x=diagnoses_all_first_denom_aurum, y=blood_pressure_aurum, by = "row_ID", all.x = TRUE) # where covariate depend on diagnosis date, merge by row_ID

# weight
diagnoses_all_first_denom_add_aurum = merge(x=diagnoses_all_first_denom_add_aurum, y=bmi_aurum, by = "row_ID", all.x = TRUE)

# smoking
diagnoses_all_first_denom_add_aurum = merge(x=diagnoses_all_first_denom_add_aurum, y=smoking_aurum, by = "row_ID", all.x = TRUE)

# cholesterol
diagnoses_all_first_denom_add_aurum = merge(x=diagnoses_all_first_denom_add_aurum, y=chol_aurum, by = "row_ID", all.x = TRUE)
format(nrow(diagnoses_all_first_denom_add_aurum), big.mark="'")

# comorbidities
diagnoses_all_first_denom_add_comord_aurum = merge(x=diagnoses_all_first_denom_add_aurum, y=comorb_aurum_first_index_long, by = "row_ID", all.x = TRUE)
format(nrow(diagnoses_all_first_denom_add_comord_aurum), big.mark="'") 
diagnoses_all_first_denom_add_comord_aurum[is.na(CKD_any_time_flag), CKD_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(CAN_any_time_flag), CAN_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(DIA2_any_time_flag), DIA2_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(DYS_any_time_flag), DYS_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(HYP_any_time_flag), HYP_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(OBE_any_time_flag), OBE_any_time_flag:=0]; diagnoses_all_first_denom_add_comord_aurum[is.na(HTH_any_time_flag), HTH_any_time_flag:=0]  # for those without relevant diagnoses, set disease flag to 0
summary(diagnoses_all_first_denom_add_comord_aurum$OBE_any_time_flag)
format(nrow(diagnoses_all_first_denom_add_comord_aurum[is.na(OBE_any_time_flag)]), big.mark="'") # check that no flag is set to NA

# Set flags for comorbidity prevalence up to index date
diagnoses_all_first_denom_add_comord_aurum[,CAN_flag := CAN_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[CAN_any_time_date > diag_date ,CAN_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,CKD_flag := CKD_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[CKD_any_time_date > diag_date ,CKD_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,DIA2_flag := DIA2_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[DIA2_any_time_date > diag_date ,DIA2_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,DYS_flag := DYS_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[DYS_any_time_date > diag_date ,DYS_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,HYP_flag := HYP_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[HYP_any_time_date > diag_date ,HYP_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,OBE_flag := OBE_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[OBE_any_time_date > diag_date ,OBE_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,PSA_flag := PSA_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[PSA_any_time_date > diag_date ,PSA_flag := 0]
diagnoses_all_first_denom_add_comord_aurum[,HTH_flag := HTH_any_time_flag]; diagnoses_all_first_denom_add_comord_aurum[HTH_any_time_date > diag_date ,HTH_flag := 0]

diagnoses_all_first_denom_add_comord_aurum[,CAN_date := CAN_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[CAN_any_time_date > diag_date ,CAN_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,CKD_date := CKD_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[CKD_any_time_date > diag_date ,CKD_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,DIA2_date := DIA2_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[DIA2_any_time_date > diag_date ,DIA2_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,DYS_date := DYS_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[DYS_any_time_date > diag_date ,DYS_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,HYP_date := HYP_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[HYP_any_time_date > diag_date ,HYP_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,OBE_date := OBE_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[OBE_any_time_date > diag_date ,OBE_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,PSA_date := PSA_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[PSA_any_time_date > diag_date ,PSA_date := NA]
diagnoses_all_first_denom_add_comord_aurum[,HTH_date := HTH_any_time_date]; diagnoses_all_first_denom_add_comord_aurum[HTH_any_time_date > diag_date ,HTH_date := NA]

# ethnicity
diagnoses_all_first_denom_add_comord_aurum = merge(x=diagnoses_all_first_denom_add_comord_aurum, y=aurum_ethnicity, by = "patid", all.x = TRUE, allow.cartesian = TRUE)
format(nrow(diagnoses_all_first_denom_add_comord_aurum), big.mark="'") # where covariate is time-independent, merge by patid

format(nrow(diagnoses_all_first_denom_aurum), big.mark="'") 
format(nrow(diagnoses_all_first_denom_add_comord_aurum), big.mark="'") 

# save table
fwrite(diagnoses_all_first_denom_add_comord_aurum, file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord_aurum.txt.gz")) 
rm(list=setdiff(ls(), c("study_start_date", "study_end_date", "linkage_start_date", "PATH", "PATH_D", "PATH_W", "PATH_W_all", "PATH_L", "PATH_P", "diagnoses_all_first_denom", "diagnoses_all_first_denom_aurum", "obs_codes", "comorb_codes_table_1", "ethnicity_codes_gold_aurum", "diagnoses_all_first_denom_add_comord_aurum", "diagnoses_all_first_denom_add_comord_gold", "comorb_gold_first", "comorb_aurum_first")))
gc()
...
#
diagnoses_all_first_denom_add_comord_gold = fread(file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord_gold.txt.gz"), colClasses = c("eventdate"="Date", "first_AD_date"="Date", "first_HER_date"="Date", "first_CVD_date"="Date", "first_INF_date"="Date", "first_CVD_hes_primary_date"="Date", "start_date"="Date", "end_date" ="Date", "lcdate" ="Date", "regdate" ="Date","todate"="Date", "diag_date"="Date", "BP_eventdate"="Date", "CAN_date"="Date", "CKD_date"="Date", "DIA2_date" ="Date", "DYS_date"="Date", "HYP_date"="Date", "OBE_date"="Date", "HTH_date"="Date"))
#
diagnoses_all_first_denom_add_comord_aurum = fread(file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord_aurum.txt.gz"), colClasses = c("eventdate"="Date", "first_AD_date"="Date", "first_HER_date"="Date", "first_CVD_date"="Date", "first_INF_date"="Date", "first_CVD_hes_primary_date"="Date", "start_date"="Date", "end_date" ="Date", "lcdate" ="Date", "regdate" ="Date","todate"="Date", "diag_date"="Date", "BP_eventdate"="Date", "CAN_date"="Date", "CKD_date"="Date", "DIA2_date" ="Date", "DYS_date"="Date", "HYP_date"="Date", "OBE_date"="Date", "HTH_date"="Date"))
#diagnoses_all_first_denom_add_comord_aurum= fread(file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord_aurum.txt.gz"))
colnames(diagnoses_all_first_denom_add_comord_gold)
summary(diagnoses_all_first_denom_add_comord_gold$HTH_flag)
head(diagnoses_all_first_denom_add_comord_aurum)
summary(diagnoses_all_first_denom_add_comord_aurum$deathdate)
summary(diagnoses_all_first_denom_add_comord_gold$deathdate)
#diagnoses_all_first_denom_add_comord_aurum[,deathdate:=NULL]
colnames(diagnoses_all_first_denom_add_comord_aurum)
colnames(diagnoses_all_first_denom_add_comord_gold)
class(diagnoses_all_first_denom_add_comord_gold$CVD_as_first_death_cause_date)
class(diagnoses_all_first_denom_add_comord_aurum$CVD_as_first_death_cause_date)
diagnoses_all_first_denom_add_comord = rbind(diagnoses_all_first_denom_add_comord_gold, diagnoses_all_first_denom_add_comord_aurum, fill =TRUE)

diagnoses_all_first_denom_add_comord[ethnicity_description=="", ethnicity_description:= NA]
diagnoses_all_first_denom_add_comord[ethnicity_source=="", ethnicity_source:= NA]
diagnoses_all_first_denom_add_comord[ethnicity_category1=="", ethnicity_category1:= NA]
diagnoses_all_first_denom_add_comord[ethnicity_category2=="", ethnicity_category2:= NA]
diagnoses_all_first_denom_add_comord[smok_status_name=="", smok_status_name:= NA]

fwrite(diagnoses_all_first_denom_add_comord, file.path(PATH_W,"CVD_diagnoses_all_first_denom_add_comord.txt.gz")) 
```
